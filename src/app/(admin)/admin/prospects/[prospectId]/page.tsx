  ...
  function renderNextAction() {
    ...
    if (status === 'agreement_sent') {
      return banner(
        ...
        `Sent to ${prospect.email ?? 'the consignor'} at ${prospect.agreedCommissionPercent ?? commissionPercent}% commission. Resend if it was missed, or create lots without a signature from the panel below.`,
        (
    ...
    if (status === 'agreement_signed') {
      ...
      return banner('green', `Agreement signed ${formatDate(prospect.agreementSignedAt)}`, catalogedCount > 0 ? 'Accept the cataloged items below to create lots.' : 'No accepted items are waiting for lot creation.', null);
    }

    if (catalogedCount > 0 && acceptedCount === 0) {
      return banner('yellow', `Next step: Review and accept ${catalogedCount} cataloged item${catalogedCount !== 1 ? 's' : ''}`, 'Review AI suggestions below, then accept or decline each item. Or bulk accept all.', (
    ...
    if (hasAcceptedItems) {
      ...
      return banner('orange', 'Next step: Record the agreement', `${acceptedCount} item${acceptedCount !== 1 ? 's' : ''} accepted but there is no email on file. Add one to send the agreement, or create lots with the commission below if the terms were agreed offline.`, (
        <Button size="sm" variant="outline" onClick={openContactEdit}>
          <Pencil className="h-4 w-4 mr-2" /> Edit contact
        </Button>
      ));
    ...
    if (status === 'upload_sent') {
      return banner(
        ...
        prospect.email ? "Resend the link if they haven't received it, or copy it from the chips above to share another way." : 'No email on file — copy the link above and share it directly.',
        (
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-3 mb-1">
              ...
              <Button variant="ghost" size="sm" className="h-7 px-2 text-muted-foreground" onClick={openContactEdit} aria-label="Edit contact">
                <Pencil className="h-3.5 w-3.5 mr-1" /> Edit contact
              </Button>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {prospect.status !== 'archived' && (
              <Button variant="ghost" size="sm" onClick={requestArchive} title="Archive prospect">
                <Archive className="h-4 w-4 mr-2" /> Archive
              </Button>
            )}
            <Button
              ...
              title={lotsExist ? 'Prospects with lots cannot be deleted' : 'Delete prospect'}
            >
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {agreementSigned ? (
            ...
            <p className="text-xs text-muted-foreground">Accept items above before sending an agreement.</p>
          ) : null}
        </CardContent>
      </Card>

      {/* ── 5. Auction Placement Panel ── */}
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          {!hasAcceptedItems ? (
            <p className="text-xs text-muted-foreground">Accept items above before creating lots.</p>
          ) : !agreementSigned ? (
            <p className="text-xs text-muted-foreground">
              The agreement isn&apos;t signed yet — you&apos;ll be asked to confirm creating lots at the commission above.
            </p>
          ) : null}
        </CardContent>
      </Card>
