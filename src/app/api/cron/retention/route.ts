...
import { db } from '@/db';
import { micrositeEvents, webhookLogs } from '@/db/schema';
 ...
 * payload after 30 days and the row after 90. Microsite events are kept for
 * 13 months so the year-over-year view still has last season.
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const deletedEvents = await db
      .delete(micrositeEvents)
      .where(lt(micrositeEvents.createdAt, new Date(now - 395 * DAY)))
      .returning({ id: micrositeEvents.id });

    const result = {
      webhookLogsDeleted: deletedWebhooks.length,
      webhookPayloadsTrimmed: trimmedPayloads.length,
      micrositeEventsDeleted: deletedEvents.length,
    };
