...
export interface Microsite {
  ...
  faqs: MicrositeFaq[];
  /**
   * This city's own tracking number, answered by the voice concierge
   * (agents/voice-agent), which reads the dialled number to know which town
   * the caller found. Leave unset to show the main Mayells line; set it only
   * once the number is live in LiveKit, or the page advertises a dead line.
   * `metaDescription` spells the number out too; update it in the same edit.
   */
  phone?: SitePhone;
}

export interface SitePhone {
  /** As printed, e.g. "(561) 555-0100". */
  display: string;
  /** E.164, e.g. "+15615550100" — what LiveKit reports as the dialled number. */
  e164: string;
}
...
const BY_NUMBER = new Map(
  MICROSITES.flatMap((m) => (m.phone ? [[m.phone.e164, m] as const] : [])),
);

/** The city whose tracking number was dialled, if it was one of them. */
export function getMicrositeByNumber(e164: string | null | undefined): Microsite | undefined {
  if (!e164) return undefined;
  return BY_NUMBER.get(e164.replace(/[^\d+]/g, ''));
}

/** The number a city page shows: its own tracking line, or the main one. */
export function micrositePhone(site: Microsite): { display: string; href: string } {
  if (site.phone) return { display: site.phone.display, href: `tel:${site.phone.e164}` };
  return { display: BUSINESS.phone, href: BUSINESS.phoneHref };
}
